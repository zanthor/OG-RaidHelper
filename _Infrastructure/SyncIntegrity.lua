...
-- Hash string to number (simple CRC-style)
...
-- Simple table to string serializer
...
function OGRH.ComputeRaidChecksum(raidName)
    ...
    local raidCopy = OGRH.DeepCopy(targetRaid)
...
-- Compute checksum for global role assignments (TANKS, HEALERS, MELEE, RANGED)
function OGRH.CalculateRolesUIChecksum()
    ...
    return OGRH.HashString(roleString)
end

-- Compute checksums for global components (consumes, tradeItems)
function OGRH.GetGlobalComponentChecksums()
    ...
    if sv.consumes then
        ...
        checksums.consumes = OGRH.HashString(serialized)
    else
    ...
    if sv.tradeItems then
        ...
        checksums.tradeItems = OGRH.HashString(serialized)
    else
...
-- Compute Active Assignments checksum for a specific encounter
function OGRH.ComputeActiveAssignmentsChecksum(encounterIdx)
    local activeRaid = OGRH.GetActiveRaid()
